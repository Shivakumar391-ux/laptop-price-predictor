...
def train_model():
    ...
    TARGET    = 'log_price'
    DROP_COLS = ['Price_euros','log_price','Ram','Weight','Cpu','Gpu',
                 'Memory','ScreenResolution','OpSys']
...
inches    = st.sidebar.selectbox("Screen Size (inches)",
    [10.1,11.6,12.0,12.5,13.3,14.0,15.6,17.3], index=4)
...
weight_cat = ("Ultralight" if weight_kg<1.5 else
              "Light" if weight_kg<2.0 else
              "Standard" if weight_kg<2.5 else "Heavy")
...
with col2:
    ...
    if st.button("Predict Price", use_container_width=True, type="primary"):
        try:
            ...
            price  = np.expm1(model.predict(X_sel)[0])
            ...
            segment = ("Budget" if price<500 else
                       "Mid-Range" if price<1000 else
                       "Premium" if price<1800 else "Ultra Premium")
            st.metric("Segment", segment)
            st.metric("Price (EUR)", f"€{price:,.0f}")
            st.metric("Range", f"€{low:,.0f} – €{high:,.0f}")
